	 ...
	 try 
        {
			...
			$qq = "insert into `".$this->className."` (".$fields.") values(".$values.")";
			//die($qq);
			$statut=connexion::getConnexion()->exec($qq);
			}
...
$sqlupdate="update ".$this->className." set ";
	...
	foreach ($this->class_vars as $name => $value) {
		if (isset($_POST[$name])){
			$sqlupdate.=$name."='".htmlspecialchars(addslashes($this->$name))."', ";
		}	
	}	
...
return $result->fetch(PDO::FETCH_ASSOC);
	...
	public function insertQuery($q){
			 ...
			 return $result=connexion::getConnexion()->exec($q);
